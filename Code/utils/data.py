...
class SalObjDataset(data.Dataset):
    ...
    def resize(self, img, gt, depth):
        ...
        if h < self.trainsize or w < self.trainsize:
            ...
            return img.resize((w, h), Image.Resampling.BILINEAR), gt.resize((w, h), Image.Resampling.NEAREST), depth.resize((w, h), Image.Resampling.NEAREST)
        else:
            return img, gt, depth
...
class SalObjDataset_var(data.Dataset):
    ...
    def resize(self, img, gt, depth):
        ...
        if h < self.trainsize or w < self.trainsize:
            h = max(h, self.trainsize)
            w = max(w, self.trainsize)
            return img.resize((w, h), Image.Resampling.BILINEAR), gt.resize((w, h), Image.Resampling.NEAREST), depth.resize((w, h),
...
class TestDataset(Dataset):
    def __init__(self, image_root, gt_root, depth_root, testsize):
        self.testsize = testsize
        self.images = sorted(
            [os.path.join(image_root, f) for f in os.listdir(image_root) if f.endswith(('.jpg', '.png'))])
        self.gts = sorted([os.path.join(gt_root, f) for f in os.listdir(gt_root) if f.endswith(('.jpg', '.png'))])
        self.depths = sorted(
            [os.path.join(depth_root, f) for f in os.listdir(depth_root) if f.endswith(('.jpg', '.png'))])
    ...
    def __getitem__(self, idx):
        ...
        # Load depth
        depth = Image.open(self.depths[idx]).convert('RGB')
        depth = self.depth_transform(depth)

        return image, depth, gt, name, orig_size
