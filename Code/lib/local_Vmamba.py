...
from Code.lib.SGE import SpatialGroupEnhance
class VSSBlock(nn.Module):
    def __init__(
        ...
        # 根据use_tse决定是否初始化SpatialGroupEnhance
        if use_tse:
            self.tse = SpatialGroupEnhance(groups=8)
        else:
            self.tse = None
    ...
    def _forward(self, input: torch.Tensor):
        ...
        if self.tse is not None:
            x = self.tse(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
