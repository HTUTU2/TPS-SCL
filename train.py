import os
import torch
import torch.nn.functional as F
import numpy as np
from datetime import datetime
from Code.lib.model import ALSOD
from Code.utils.data import SalObjDataset, test_dataset
from Code.utils.utils import clip_gradient, adjust_lr
import logging
import torch.backends.cudnn as cudnn
from Code.utils.options import opt
import torch.nn as nn
from smooth_loss import get_saliency_smoothness
import random
from torch.utils.data import DataLoader
import Code.utils.misc as utils
from Code.utils.default import _C as cfg

cudnn.benchmark = True

cfg.TRAIN.distributed = False

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
seed = cfg.TRAIN.seed + utils.get_rank()
torch.manual_seed(seed)
np.random.seed(seed)
random.seed(seed)

model = ALSOD()

rgb_pretrained_path = './Pretrained model/swin_base_patch4_window12_384_22k.pth'
t_pretrained_path = './Pretrained model/swin_base_patch4_window12_384_22k.pth'
#
# model.load_pre(rgb_pretrained_path, t_pretrained_path)



if (opt.load is not None):
    model.load_pre(opt.load)
    print('load model from ', opt.load)

model.to(device)
n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
print('number of params:', n_parameters)

params = model.parameters()
optimizer = torch.optim.AdamW(params, lr=opt.lr, weight_decay=1e-4)

lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, 50)

train_image_root = opt.rgb_label_root
train_gt_root = opt.gt_label_root
train_depth_root = opt.depth_label_root

val_image_root = opt.val_rgb_root
val_gt_root = opt.val_gt_root
val_depth_root = opt.val_depth_root
save_path = opt.save_path

if not os.path.exists(save_path):
    os.makedirs(save_path)

print('load data...')
dataset_train = SalObjDataset(train_image_root, train_gt_root, train_depth_root, trainsize=opt.trainsize)
dataset_test = test_dataset(val_image_root, val_gt_root, val_depth_root, opt.trainsize)

sampler_train = torch.utils.data.RandomSampler(dataset_train)
batch_sampler_train = torch.utils.data.BatchSampler(sampler_train, opt.batchsize, drop_last=True)
train_loader = DataLoader(dataset_train, batch_sampler=batch_sampler_train, num_workers=16, pin_memory=True)

total_step = len(train_loader)

logging.basicConfig(filename=save_path + 'log.log', format='[%(asctime)s-%(filename)s-%(levelname)s:%(message)s]',
                    level=logging.INFO, filemode='a', datefmt='%Y-%m-%d %I:%M:%S %p')
logging.info("BBSNet_unif-Train")
logging.info("Config")
logging.info('epoch:{};lr:{};batchsize:{};trainsize:{};clip:{};decay_rate:{};load:{};save_path:{};decay_epoch:{}'.format(
    opt.epoch, opt.lr, opt.batchsize, opt.trainsize, opt.clip, opt.decay_rate, opt.load, save_path, opt.decay_epoch))

CE = torch.nn.BCEWithLogitsLoss()

step = 0
best_mae = 1
best_epoch = 0

print(len(train_loader))


class SoftDiceLoss(nn.Module):
    def __init__(self, weight=None, size_average=True):
        super(SoftDiceLoss, self).__init__()

    def forward(self, logits, targets):
        num = targets.size(0)
        smooth = 1

        probs = torch.sigmoid(logits)
        m1 = probs.view(num, -1)
        m2 = targets.view(num, -1)
        intersection = (m1 * m2)

        score = 2. * (intersection.sum(1) + smooth) / (m1.sum(1) + m2.sum(1) + smooth)
        score = 1 - score.sum() / num
        return score

dice = SoftDiceLoss()

def train(train_loader, model, optimizer, epoch, save_path):
    global step
    model.train()
    loss_all = 0
    epoch_step = 0

    try:
        # 在每个epoch开始前获取并打印学习率
        current_lr = optimizer.param_groups[0]['lr']
        print('Epoch [{:03d}/{:03d}] Starting, lr: {:.8f}'.format(epoch, opt.epoch, current_lr))
        logging.info('Epoch [{:03d}/{:03d}] Starting, lr: {:.8f}'.format(epoch, opt.epoch, current_lr))
        for i, (images, gts, depths) in enumerate(train_loader, start=1):
            optimizer.zero_grad()

            images = images.to(device)
            gts = gts.to(device)
            depths = depths.to(device)

            # 前向传播
            out= model(images, depths)


            sml1 = get_saliency_smoothness(torch.sigmoid(out), gts)
            sml = sml1

            loss_fusion1 = F.binary_cross_entropy_with_logits(out, gts)
            loss_fusion = loss_fusion1

            dice_loss1 = dice(out, gts)
            dice_loss = dice_loss1

            loss_seg = loss_fusion + sml + dice_loss
            loss = loss_seg
            loss.backward()

            clip_gradient(optimizer, opt.clip)
            optimizer.step()
            # 获取当前学习率
            step += 1
            epoch_step += 1
            loss_all += loss.item()
            if i % 50 == 0 or i == total_step or i == 1:
                print(
                    '{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], loss: {:.4f} sml: {:.4f} loss1_fusion: {:0.4f} dice: {:0.4f} '.
                    format(datetime.now(), epoch, opt.epoch, i, total_step, loss.item(), sml.item(), loss_fusion.item(), dice_loss.item()))
                logging.info(
                    '#TRAIN#:Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], loss: {:.4f} sml: {:.4f} loss1_fusion: {:0.4f} dice: {:0.4f}  '.
                    format(epoch, opt.epoch, i, total_step, loss.item(), sml.item(), loss_fusion.item(), dice_loss.item()))
        loss_all /= epoch_step
        logging.info('#TRAIN#:Epoch [{:03d}/{:03d}], Loss_AVG: {:.4f}'.format(epoch, opt.epoch, loss_all))

        if (epoch) % 1 == 0 and epoch >= 10:
            torch.save(model.state_dict(), save_path + 'HyperNet_epoch_{}.pth'.format(epoch))
            # 更新学习率
        lr_scheduler.step()
    except KeyboardInterrupt:
        print('Keyboard Interrupt: save model and exit.')
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        torch.save(model.state_dict(), save_path + 'HyperNet_epoch_{}.pth'.format(epoch + 1))
        print('save checkpoints successfully!')
        raise


def val(test_loader, model, epoch, save_path):
    global best_mae, best_epoch
    model.eval()
    with torch.no_grad():
        mae_sum = 0
        for i in range(test_loader.size):
            image, gt, depth, name, img_for_post = test_loader.load_data()
            gt = np.asarray(gt, np.float32)
            gt /= (gt.max() + 1e-8)
            image = image.to(device)
            depth = depth.to(device)

            out= model(image, depth)

            if not isinstance(out, torch.Tensor):
                raise ValueError(f"Expected 'out' to be a torch.Tensor, but got {type(out)}")

            res = out
            res = F.interpolate(res, size=gt.shape, mode='bilinear', align_corners=False)
            res = res.sigmoid().cpu().numpy().squeeze()
            res = (res - res.min()) / (res.max() - res.min() + 1e-8)
            mae_sum += np.sum(np.abs(res - gt)) * 1.0 / (gt.shape[0] * gt.shape[1])

        mae = mae_sum / test_loader.size
        print('Epoch: {} MAE: {} ####  bestMAE: {} bestEpoch: {}'.format(epoch, mae, best_mae, best_epoch))
        if epoch == 1:
            best_mae = mae
        else:
            if mae < best_mae:
                best_mae = mae
                best_epoch = epoch
                torch.save(model.state_dict(), os.path.join(save_path, 'SPNet_epoch_best.pth'))
                print('best epoch:{}'.format(epoch))

        logging.info('#TEST#:Epoch:{} MAE:{} bestEpoch:{} bestMAE:{}'.format(epoch, mae, best_epoch, best_mae))

if __name__ == '__main__':
    print("Start train...")
    try:
        for epoch in range(1, opt.epoch + 1):
            train(train_loader, model, optimizer, epoch, save_path)
            if epoch >= 0:
                val(dataset_test, model, epoch, save_path)
    except Exception as e:
        print(f"error: {e}")
        import traceback
        traceback.print_exc()
